    ...
    helmet({
      contentSecurityPolicy: {
        ...
        directives: {
          ...
          scriptSrc: ["'self'", `'nonce-${res.locals.nonce}'`, config.application.url].concat(
            config.cspDirectives.scriptSrc
          ),
...
const serverListeningHandler = () => {
  logger.info(
    `Express server listening on port ${server.address().port} in ${app.settings.env} mode with ${config.application.useLocalSSLCert ? 'local' : 'managed'} SSL cert`
  );
    ...
    (async () => {
      ...
      if (config.marvel.fetchCharacter) {
        ...
        if (marvelCharacter) {
          ...
          await ConfigModel.set('MARVEL_CHARACTER', JSON.stringify(marvelCharacter)).catch((e) =>
            logger.error('Save Marvel character failed', e)
          );
        }
      }
      if (config.marvel.fetchComics) {
        ...
        if (marvelComics) {
          ...
          await ConfigModel.set('MARVEL_COMICS', JSON.stringify(marvelComics)).catch((e) =>
            logger.error('Save Marvel comics failed', e)
          );
        }
      }
...
if (config.application.useLocalSSLCert) {
  server = https
    ...
    .listen(config.port, serverListeningHandler)
