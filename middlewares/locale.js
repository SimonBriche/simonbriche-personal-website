  ...
  if (localeDomainsCache === undefined && localeSubfoldersCache === undefined) {
    if (typeof config.application.localeDomains === 'string') {
      try {
        ...
        _logger.error(
          'Your localeDomains is not a valid JSON with [{"domain":"YOUR_DOMAIN","locale":"lo_lo"}] format. Fix it or delete the ENV.LOCALE_DOMAINS. Your current localeDomains is',
          config.application.localeDomains
        );
      }
      ...
      try {
        ...
        _logger.error(
          'Your localeSubfolders is not a valid JSON with [{"folder":"folder","locale":"lo_lo"}] format. Fix it or delete the ENV.LOCALE_SUBFOLDERS. Your current localeSubfolders is',
          config.application.localeSubfolders
        );
      }
    } else {
