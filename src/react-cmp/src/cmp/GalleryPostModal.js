...
const GalleryPostModal = (props) => {
  ...
  useEffect(() => {
    if (isBootstrapAvailable('5')) {
      setModal(
        new window.bootstrap.Modal(el.current, {
          keyboard: false,
          backdrop: false,
        })
      );
    ...
    if (bridge && modal) {
        ...
        if (!allTechnologies) {
          allTechnologies = new Promise(function (resolve, reject) {
            (async () => {
              ...
              const res = await (
                await fetch(`${process.env.REACT_APP_CDN_URL}/graphql?query=${encodeURIComponent(query)}`)
              ).json();
      ...
      bridge.addEventListener('openGalleryModal', (e) => {
        ...
        getTechnologies()
          .then((technologies) => {
            setTechnologies(
              e.detail.technology_ids.map((technologyId) => technologies.find((item) => item.id === technologyId))
            );
          })
          .catch((e) => {
            ...
            {modalInfos && (
              <div className="card portfolio-focus soft shadow rounded border-0 mx-sm-auto">
                <img
                  ...
                  src={`${process.env.REACT_APP_CDN_URL}/assets/images/gallery/${modalInfos.thumbnail}`}
                  alt={modalInfos.name}
                />
                  ...
                  {modalInfos.description && (
                    <>
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{ __html: modalInfos.description }}></div>
                    </>
                  )}
                  <div id="portfolio-carousel" className="carousel slide" data-bs-ride="carousel">
                    <div className="carousel-inner">
                      {modalInfos.images &&
                        modalInfos.images.map((image, index) => (
                          <div
                            className={`carousel-item ${index === 0 ? 'active' : ''}`}
                            key={`carousel-image-${index}`}>
                            <img
                              className="d-block w-100 lazyload fade"
                              data-src={`${process.env.REACT_APP_CDN_URL}/assets/images/gallery/${image}`}
                              alt={`${modalInfos.client}-${index}`}
                              loading="lazy"
                            />
                          </div>
                        ))}
                          ...
                          <li className="list-inline-item" key={`carousel-technology-${index}`}>
                            <a
                              ...
                              <img
                                ...
                                data-src={`${process.env.REACT_APP_CDN_URL}/assets/images/stack/${technology.thumbnail}`}
                                alt={`${technology.name}-${index}`}
                                loading="lazy"
                              />
                            </a>
                          </li>
