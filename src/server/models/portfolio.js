...
const {
  ...
  getPageInfo,
  generateFilterQuery,
  generateOrderAndFilterQuery,
} = require('../utils/graphql-util');
...
module.exports = {
  getByCursor: function (id, fields, pagingInfos) {
    return new Promise(function (resolve, reject) {
      (async function () {
        ...
        const defaultFields = ['portfolio.id', 'portfolio.name', 'portfolio.description'];
        if (fields) {
        ...
        if (id) {
          ...
          const { pageInfo, data } = getPageInfo(portfolio, {
            first: first,
            last: last,
            limit: limit,
            cursor: cursor,
            orderBy: pagingInfos.orderBy,
          });
  ...
  getCount: function (filtering) {
    return new Promise(function (resolve, reject) {
      const query = db('portfolio').count('id as count').first();
  ...
  getTechnologies: function (fields) {
    return new Promise(function (resolve, reject) {
      (async function () {
        const defaultFields = ['technologies.id', 'technologies.name'];
        if (fields) {
          //ensure that id is requested
          fields.push('id');
          //format fields to map them with the DB fields
  ...
  getPortfolioSearchFields: function (fields) {
    return new Promise(function (resolve, reject) {
      (async function () {
        const defaultFields = ['clients', 'types'];
        const requestedFields = fields || defaultFields;
        const searchFields = {};
        if (requestedFields.indexOf('clients') !== -1) {
          const clientsQuery = await db('portfolio').distinct('client').orderBy('client', 'ASC');
          searchFields.clients = clientsQuery.map((item) => item.client);
        }
        if (requestedFields.indexOf('types') !== -1) {
          const clientsQuery = await db('portfolio').distinct('types');
          searchFields.types = [
            ...new Set(
              clientsQuery.map((item) => JSON.parse(item.types)).reduce((prev, curr) => prev.concat(curr), [])
            ),
