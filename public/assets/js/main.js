...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (modalProfileEl && window.bootstrap) {
    ...
    modalProfileEl.addEventListener('hidden.bs.modal', function (event) {
      modalProfileEl
        .querySelectorAll('.youtube_player iframe')
        .forEach((item) => (item.attributes.src.value = item.attributes.src.value));
    });
    ...
    document.querySelector('#btn-close-footer').addEventListener(
      'click',
      () => {
        document.querySelectorAll('.footer-interface').forEach((item) => item.classList.toggle('open'));
...
window.CustomApp.formOptions = {
  ...
  invalidHandler: function (event, validator) {
    ...
    if (errors) {
      window.CustomApp.showNotif(
        'Il y a <strong>' + errors + ' erreur' + (errors > 1 ? 's' : '') + '</strong> à corriger dans le formulaire.',
        'danger'
      );
    }
  },
