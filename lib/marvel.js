...
const Marvel = {
  ...
  api: function (endpoint, params, accessToken, method = 'GET') {
    return new Promise(async (resolve, reject) => {
      ...
      const hash = crypto
        .createHash('md5')
        .update(timestamp + accessToken.private + accessToken.public)
        .digest('hex');
      let url =
        endpoint.substr(0, 4) === 'http' ?
          endpoint
        : `${config.marvel.apiGateway}${endpoint}?ts=${timestamp}&apikey=${accessToken.public}&hash=${hash}`;
      ...
      requestOptions.https = Object.assign(requestOptions.https || {}, {
      ...
      _logger.verbose('get marvel api from : ', {
        ...
        https: requestOptions.https,
      });

      (await got)(requestOptions).json().then(resolve, reject);
    });
  },
  /**
          ...
          if (totalItems > 0) {
            ...
            if (items && items.data && items.data.results && items.data.results.length > 0) {
              ...
              const randomItemWithPictureAndDescription = randomItems
                .filter(
                  (item) =>
                    item.thumbnail && item.thumbnail.path && item.thumbnail.path.indexOf('image_not_available') === -1
                )
                .filter((item) => !!item.description)[0];
              const randomItemWithPicture = randomItems.filter(
                (item) =>
                  item.thumbnail && item.thumbnail.path && item.thumbnail.path.indexOf('image_not_available') === -1
              )[0];
              const randomItemWithDescription = randomItems.filter((item) => !!item.description)[0];

              const randomItem =
                randomItemWithPictureAndDescription ||
                randomItemWithPicture ||
                randomItemWithDescription ||
                randomItems[0];

              if (isFormatted) {
                const thumbnailPath =
                  randomItem.thumbnail && randomItem.thumbnail.path && randomItem.thumbnail.extension ?
                    randomItem.thumbnail.path.replace('http://', 'https://')
                  : '';

                const thumbnail =
                  !thumbnailPath ? ''
                  : thumbnailPath.indexOf('image_not_available') !== -1 ?
                    thumbnailPath + '.' + randomItem.thumbnail.extension
                ...
                const formattedItem = {
                  ...
                  description:
                    randomItem.description ?
                      randomItem.description.replace(/(\r\n|\r|\n)/g, '<br>')
                    : randomItem.description,
                  thumbnail: thumbnail,
