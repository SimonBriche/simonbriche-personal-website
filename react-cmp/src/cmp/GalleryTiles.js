    ...
    <div className="gallery-container container-fluid">
      <div className="row justify-content-center" ref={el}>
        {galleryTiles ?
            ...
            galleryTiles.map((item) => (
              <div className="col-12 col-sm-6 col-md-4 col-lg-3 mb-3" key={item.id}>
                <div
                  ...
                  onClick={(e) => postClickHandler(e, item)}>
                  <img
                    className="card-img-top rounded-top lazyload fade"
                    data-src={`${process.env.REACT_APP_CDN_URL}/assets/images/gallery/${item.thumbnail}`}
                    alt={item.name}
                    loading="lazy"
                  />
          ...
          : <div className="col-12 text-center">
              <div className="soft d-inline-block my-2 p-3 rounded">
                Z...zut ! Aucune réalisation ne correspond à cette recherche 😅
              </div>
            </div>

        : <div className="col-12 text-center">
            <button onClick={() => showMoreHandler()} className="btn btn-custom btn-soft">
              Lancez une première recherche 🔍
            </button>
          </div>
        }
      </div>
      <div className="text-center mt-3">
        {galleryTiles && galleryTiles.length > 0 && (
          <button onClick={scrollToFilters} className="btn btn-custom btn-soft m-3">
            Retour au filtres
          </button>
        )}
        {showMore && (
          <button onClick={() => showMoreHandler(afterCursor)} className="btn btn-custom btn-soft m-3">
            Plus de projets
          </button>
        )}
      </div>
