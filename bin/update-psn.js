...
const optionDefinitions = [{ name: 'npsso' }, { name: 'help', alias: 'h' }];
    ...
    optionList: [
      {
        name: 'npsso',
        description:
          'First, login to your PSN account at https://www.playstation.com/. Once connected, open a new browser tag and go to https://ca.account.sony.com/api/v1/ssocookie. A JSON will appear with the npsso as a property.',
      },
    ],
  },
];
...
if (argv && argv.npsso) {
  (async () => {
    ...
    if (trophies && trophies.trophyTitles && trophies.trophyTitles.length > 0) {
      console.log(
        'titles',
        trophies.trophyTitles.map((item) => ({
